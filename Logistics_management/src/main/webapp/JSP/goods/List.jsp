...
<div class="weadmin-body">
    <div class="layui-row">
        <div class="layui-inline">
            <input type="text" name="Goods_Name" id="Goods_Name" placeholder="请输入商品名" autocomplete="off"
                   class="layui-input">
        </div>
        <div class="layui-inline">
            <select class="layui-select" id="Goods_Type_Id">
...
<form method="post" action="${pageContext.request.contextPath}/delAllwarehouse">
    <div class="weadmin-block">
        <input type="button" class="layui-btn" value="添加"
               onclick="WeAdminShow('添加商品','${pageContext.request.contextPath}/JSP/goods/Add.jsp',600)">
...
<script type="text/javascript">
    function myreset(){
        $('#Goods_Name').val("");
        $('#Goods_Type_Id').find("option").eq(0).prop("selected",true);
    }
    $(function () {
        $.ajax({
            type: "POST",
    ...
    function query(i) {
        $.ajax({
            ...
            type: "POST",
            url: "${pageContext.request.contextPath}/listG",
            success: function (data) {
                var result = jQuery.parseJSON(data);
                var j = 1;
                var p = i + 1;
                ...
                result.forEach(function (element) {
                    ...
                    $('#memberList').append("<td>" + (j++) + "</td><td>" + element.goods_Code + "</td><td>" + element.goods_Name + "</td><td>" + element.goods_Price + "</td><td>" + element.goods_Type_Id + "</td>");
    ...
    function querynumber(i) {
        $.ajax({
            ...
            type: "POST",
            url: "${pageContext.request.contextPath}/listGNumber",
            success: function (data) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(result.j == 0){
                        ...
                        layer.msg("库存管理中有"+result.j+"条相关信息，不能删除该商品", {
                            icon: 1,
                            time: 1000
                        });
                    }
